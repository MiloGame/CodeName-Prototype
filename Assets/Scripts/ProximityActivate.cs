...
public class ProximityActivate : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (enableInfoPanel && activeState)
        {
            infoPanel.alpha = Mathf.MoveTowards(infoPanel.alpha, 1, Time.deltaTime * 10);
        }
        else
        {
            infoPanel.alpha = Mathf.MoveTowards(infoPanel.alpha, 0, Time.deltaTime * 10);
        }

        if (lookAtActivator != null)
        {
            if (activeState)
                targetRotation = Quaternion.LookRotation(lookAtActivator.position - transform.position);
            else
